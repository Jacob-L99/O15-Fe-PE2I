...
from Beräkningar import beräkning
...
def flow_regions():
    ...
    initial_window.update_idletasks()  # Force update of the label text
    V_a_reshape_list_1, K_2_reshape_list_1, K_1_reshape_list_1= beräkning(AIF_1, AIF_time_1, corrected_data_1)
    ...
    if num_selections==2:
        ...
        loading_label.config(text="Beräknar")
        initial_window.update_idletasks()  # Force update of the label text
        V_a_reshape_list_2, K_2_reshape_list_2, K_1_reshape_list_2= beräkning(AIF_2, AIF_time_2, corrected_data_2)
        loading_label.config(text="Transformerar")
        initial_window.update_idletasks()  # Force update of the label text
...
# initial_window.title("AMANDAV: står för den stakaste tjejen jag vet och min gymchrush❤️❤️❤️, hon kommer ta 200kg före Joar!")
...
def initial():
    ...
    # Dropdown menu for "Change Template" with a fixed width
    template_var = tk.StringVar()
    template_var.set("Flow regions")  # default value

    change_template_menu = tk.OptionMenu(bottom_frame, template_var, "Flow regions", "Neurodegenerativ")
    change_template_menu.config(fg='white', bg='grey', width=15)  # Fixed width for consistent size
    change_template_menu.pack(pady=10)
